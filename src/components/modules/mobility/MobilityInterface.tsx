...
import { Accessibility, MapPin, RotateCcw, Send } from "lucide-react";
import { useState } from "react";

import { AccessiblePlacesPanel } from "@/components/modules/mobility/AccessiblePlacesPanel";
...
type MobilityTab = "communicate" | "places";

const MOBILITY_TABS = [
  { id: "communicate" as const, label: "Comunicar", icon: Accessibility },
  { id: "places" as const, label: "Lugares", icon: MapPin },
];

export function MobilityInterface({ module = mobilityModule }: ModuleViewProps) {
  const [activeTab, setActiveTab] = useState<MobilityTab>("places");
  ...
  return (
    ...
    >
      ...
      {activeTab === "places" ? (
        <div id="panel-places" role="tabpanel" aria-labelledby="tab-places">
          <AccessiblePlacesPanel />
        </div>
      ) : (
        <div
          id="panel-communicate"
          role="tabpanel"
          aria-labelledby="tab-communicate"
          className="space-y-6"
        >
